...
export const History: React.FC<HistoryProps> = ({ onBack }) => {
  ...
  return (
    <div className={`min-h-screen ${colors.background.page} p-4 pt-16`}>
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          <h1 className={`text-2xl font-bold ${colors.text.primary}`}>历史记录</h1>
          <div className="flex items-center space-x-4">
            <label className="flex items-center space-x-2">
              ...
              />
              <span className={`text-sm ${colors.text.secondary}`}>最佳纪录</span>
            </label>
            <button
        ...
        <div className={`${colors.background.primary} rounded-xl shadow p-4 overflow-x-auto`}>
          <table className="min-w-full text-sm">
            <thead>
              <tr className={`text-left ${colors.text.tertiary} ${colors.border.primary} border-b`}>
            ...
            <tbody>
              {displayRecords.length === 0 && (
                <tr><td className="py-3 text-gray-500 dark:text-gray-400" colSpan={8}>暂无记录</td></tr>
              )}
              {displayRecords.map(r => (
                <tr key={r.id} className={`${colors.border.primary} border-b last:border-b-0`}>
                  <td className={`py-2 pr-4 whitespace-nowrap ${colors.text.primary}`}>{new Date(r.createdAt).toLocaleString()}</td>
                  <td className="py-2 pr-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      r.isManual ? 'bg-purple-100 dark:bg-purple-900/40 text-purple-800 dark:text-purple-300' : 'bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300'
                    }`}>
                      {getQuestionTypeName(r.type)}
                      {r.isManual && ' (手动)'}
                    </span>
                  </td>
                  <td className={`py-2 pr-4 ${colors.text.primary}`}>{r.questionCount}</td>
                  <td className={`py-2 pr-4 ${colors.status.success}`}>{r.correct}</td>
                  <td className={`py-2 pr-4 ${colors.status.error}`}>{r.wrong}</td>
                  <td className="py-2 pr-4">
                    <div className={`h-3 ${colors.progress.background} rounded`}>
                      <div className={`h-3 ${colors.progress.fill} rounded`} style={{ width: `${Math.min(100, Math.max(0, r.accuracy))}%` }} />
                    </div>
                    <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">{r.accuracy}%</div>
                  </td>
                  <td className="py-2 pr-4">
                    <div className={`h-3 ${colors.progress.background} rounded`}>
                      {/* 这里以10秒为可视化上限，可后续做自适应 */}
                      <div className="h-3 bg-indigo-500 dark:bg-indigo-400 rounded" style={{ width: `${Math.min(100, (r.avgTime / 10) * 100)}%` }} />
                    </div>
                    <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">{r.avgTime.toFixed(2)}s</div>
                  </td>
                  <td className="py-2 pr-4">
                    <button className={`${colors.status.info} hover:underline mr-3`} onClick={() => {
                      console.log('查看记录详情:', r);
                      console.log('questionLogs:', r.questionLogs);
                      setSelected(r);
                    }}>查看</button>
                    <button className={`${colors.status.error} hover:underline`} onClick={() => {
        ...
        <div className="mt-6 bg-white dark:bg-gray-800 rounded-xl shadow p-4">
          <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-3">错题汇总</h2>
        ...
        {selected && (
          <div className="fixed inset-0 z-40">
            ...
            <div className="absolute top-0 right-0 h-full w-full max-w-xl bg-white dark:bg-gray-800 shadow-xl p-4 overflow-y-auto">
              <div className="flex items-center justify-between mb-2">
                <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200">详细信息</h2>
                <button className="text-gray-600 dark:text-gray-400 hover:underline" onClick={() => setSelected(null)}>关闭</button>
              </div>
              ...
              <div className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">题目分析</div>
              {/* 调试信息 - 已隐藏 */}
              {false && (
                <div className="mb-4 p-2 bg-yellow-100 text-xs text-gray-700 dark:text-gray-300 rounded">
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-gray-50 dark:bg-gray-700">
                      <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">题目</th>
                      <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">对错</th>
                      <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">用时(秒)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {(!selected.questionLogs || selected.questionLogs.length === 0) && (
                      <tr><td className="py-3 text-gray-500 dark:text-gray-400 text-center" colSpan={3}>暂无题目详情</td></tr>
                    )}
                    {selected.questionLogs?.map((log, idx) => (
                      <tr key={idx} className={`${colors.table.border} border-b ${colors.table.row}`}>
        ...
        {isSummaryOpen && (
          <div className="fixed inset-0 z-50">
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white dark:bg-gray-800 w-full max-w-3xl rounded-xl shadow-xl p-4">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200">{summaryTitle}</h3>
                <button className="text-gray-600 dark:text-gray-400 hover:underline" onClick={() => setIsSummaryOpen(false)}>关闭</button>
              </div>

              <div className="text-sm text-gray-700 dark:text-gray-300 mb-3">共 {summaryItems.length} 题（含错题与慢题）。</div>

              <div className="max-h-80 overflow-y-auto bg-gray-50 dark:bg-gray-700 rounded p-3 text-sm">
        ...
        {showAddRecord && (
          <div className="fixed inset-0 z-50">
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white dark:bg-gray-800 w-full max-w-md rounded-xl shadow-xl p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200">添加记录</h3>
                <button className="text-gray-600 dark:text-gray-400 hover:underline" onClick={() => setShowAddRecord(false)}>×</button>
              </div>
...
const AddRecordForm: React.FC<AddRecordFormProps> = ({ onClose, onSuccess }) => {
  ...
  return (
    <div className="space-y-4">
      {/* 题型选择 */}
      <div>
        <label className="block text-sm font-semibold text-gray-800 dark:text-gray-200 mb-2">题型</label>
        <select
          value={questionType}
      ...
      {/* 速度输入 */}
      <div>
        <label className="block text-sm font-semibold text-gray-800 dark:text-gray-200 mb-2">平均速度（秒）</label>
        <input
          type="number"
          value={speed}
      ...
      {/* 单题时间 */}
      <div>
        <label className="block text-sm font-semibold text-gray-800 dark:text-gray-200 mb-2">单题时间（秒）</label>
        <input
          type="number"
          value={timeLimit}
